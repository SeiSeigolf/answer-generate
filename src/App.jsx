...
const MED_HEAVY = [
  // 代謝
  "律速酵素","解糖系","TCA","ATP","NADH","ピルビン酸","アセチルCoA","グリコーゲン","インスリン","グルカゴン","阻害","活性化","アロステリック","LDH","PDH","PFK","NADPH","G6PD",
  // 呼吸・循環
  "換気","PaCO2","PaO2","ヘモグロビン","酸素","呼吸","肺","心臓","心エコー","エコー","超音波","心電図","心音","脈拍","血圧","心拍","駆出率","左室","右室","弁","大動脈","僧帽弁","三尖弁","肺動脈","心筋","心膜","心内膜","心外膜","心タンポナーデ","心不全","狭心症","心筋梗塞","不整脈","頻脈","徐脈","心房細動","心室細動","ブロック",
  // 炎症・免疫
  "炎症","メディエーター","ヒスタミン","プロスタグランジン","COX","マクロファージ","肉芽腫","補体","白血球","リンパ球","好中球","IL-1","TNF","サイトカイン",
  // 薬理
  "CYP","ADME","半減期","代謝","排泄","吸収","分布",
  // 診断・検査
  "診断","鑑別","治療","禁忌","適応","検査","所見","症状","病態","機序","定義","分類","予後",
];
...
function createEngine() {
  // 問題文を意味のある単語に分割（助詞・記号を除去）
  function tokenize(text) {
    return text
      .replace(/[はをがにでもとやのへからまでより、。？?！!「」【】（）\s]/g, " ")
      .split(" ")
      .filter(w => w.length >= 2);
  }
  ...
  function scorePage(page, question) {
    ...
    // 2. 医学用語リスト一致
    for (const kw of MED_HEAVY) {
      if (kw.length >= 2 && question.includes(kw) && content.includes(kw)) {
        score += 18;
        reasons.push(`医学語「${kw}」`);
      }
    }

    // 3. 問題文のトークンとコンテンツの一致（柔軟マッチ）
    for (const tok of qTokens) {
      if (tok.length < 2) continue;
      if (heading.includes(tok)) {
        score += 15;
        reasons.push(`見出し「${tok}」`);
    ...
    // 4. 問題文全体の部分一致（長めのフレーズ）
    const qShort = question.replace(/[はをがにでもとやのへ、。？?！!\s]/g, "");
    if (qShort.length >= 3 && content.replace(/\s/g,"").includes(qShort.slice(0, 4))) {
      score += 10;
    }
  ...
  function extractClaims(searchResults, question) {
    ...
    for (const { doc, page, score, reasons } of searchResults) {
      ...
      claims.push({
        id: `claim_${page.id}`,
        text: page.content,
        evidenceDoc: doc.title,
        evidenceDocId: doc.id,
        evidencePage: page.pageNumber,
        evidenceHeading: page.heading || "",
        confidence: score >= 50 ? "high" : score >= 20 ? "mid" : "low",
        claimType: classifyEvidence(page, question),
  ...
  function buildAnswer(claims, mode) {
    ...
    if (!all.length) return {
      short: "資料内に関連する記述が見つかりませんでした。",
      standard: "授業資料に関連するページが見つかりませんでした。問題文のキーワードを確認するか、別の資料を登録してください。",
      detailed: "授業資料に関連するページが見つかりませんでした。",
      evidenceMap: []
    };

    const fmt = c => `${c.text}（${c.evidenceDoc.split(" ")[0]} p.${c.evidencePage}）`;
    const short = usedD[0]
      ? `${usedD[0].text.slice(0, 100)}… [${usedD[0].evidenceDoc.split(" ")[0]} p.${usedD[0].evidencePage}]`
      : `${all[0].text.slice(0, 100)}… [${all[0].evidenceDoc.split(" ")[0]} p.${all[0].evidencePage}]`;
    const standard = all.map(fmt).join(" ");
    const detailed = all.map(c => `【p.${c.evidencePage}（${c.evidenceDoc.split(" ")[0]}）】\n${c.text}`).join("\n\n");
    const evidenceMap = all.map(c => ({
      claimSnippet: c.text.slice(0, 55) + (c.text.length > 55 ? "…" : ""),
      source: `${c.evidenceDoc.split(" ")[0]} p.${c.evidencePage}`,
  ...
  function audit(claims, searchResults) {
    ...
    const revisedAnswer = direct.length > 0
      ? `資料根拠あり: ${direct.slice(0, 2).map(c => c.text.slice(0, 60) + "…").join("　")}`
      : "資料に十分な根拠が見つかりませんでした。";
...
const LS = { docs: "emab_docs_v5", active: "emab_active_v5", q: "emab_q_v5" };
...
function PdfUploadButton({ onDone }) {
  ...
  async function handleFile(e) {
    ...
    try {
      ...
      onDone({ id: uid(), title: name, courseName: name, lectureNumber: 1, year: new Date().getFullYear(), color: "#534AB7", pages });
    } catch (err) {
  ...
  return (
    <div>
      <label style={{ display: "block", width: "100%", padding: "10px 0", border: "0.5px dashed #a78bfa", borderRadius: 8, fontSize: 12, color: "#534AB7", background: "#f5f3ff", cursor: loading ? "not-allowed" : "pointer", textAlign: "center", fontWeight: 600, opacity: loading ? 0.6 : 1, boxSizing: "border-box" }}>
...
function DocModal({ doc, onClose, onSave }) {
  ...
  function parsePgs(text) {
    return text.split("\n").filter(l => l.trim()).map((line, i) => {
      ...
      if (m) { const content = m[2].trim(); return { id: uid(), pageNumber: parseInt(m[1]), heading: "", content, tags: MED_HEAVY.filter(k => content.includes(k)).slice(0, 8) }; }
